...
def run_cae(cae_model, img_array, input_tensor, threshold):
    ...
    with st.expander("ℹ️ Model Details"):
        st.write("**Architecture** : Convolutional Autoencoder")
        st.write("**Input Size** : 128 × 128")
        st.write("**Dataset** : CBIS-DDSM")
        st.write("**Loss Function** : Mean Squared Error")
        st.write("**Optimizer** : Adam")
        st.write("**Output** : Reconstruction Image")
    ...
    with col2:
        st.metric("Threshold", "1.2630%")
    with col3:
        st.metric("Evaluation", "Reconstruction Error")

    if is_anomaly:
        st.warning(
            "The anomaly score exceeds the predefined threshold, indicating abnormal "
            "reconstruction patterns in the mammogram image."
        )
    else:
        st.success(
            "The anomaly score is below the predefined threshold, indicating normal "
...
st.caption(
    ...
    "The analysis results are generated by deep learning models and "
